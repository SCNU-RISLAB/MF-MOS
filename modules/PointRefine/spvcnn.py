...
class FeatureExtractBlock(nn.Module):
    def __init__(self, inc, outc, ks=3, stride=1, dilation=1):
        ...
        self.conv113 = nn.Sequential(
            spnn.Conv3d(inc, outc, kernel_size=(1, 1, ks), dilation=dilation, stride=stride),
            spnn.BatchNorm(outc),
            spnn.ReLU(True)
        )
        self.conv131 = nn.Sequential(
            spnn.Conv3d(inc, outc, kernel_size=(1, ks, 1), dilation=dilation, stride=stride),
            spnn.BatchNorm(outc),
            spnn.ReLU(True)
        )
        self.conv311 = nn.Sequential(
            spnn.Conv3d(inc, outc, kernel_size=(ks, 1, 1), dilation=dilation, stride=stride),
            spnn.BatchNorm(outc),
            spnn.ReLU(True)
        )
    ...
    def forward(self, x):
        out = self.relu(self.conv_cat(torchsparse.cat([self.conv113(x), self.conv131(x), self.conv311(x)])) + self.downsample(x))
        return out
...
class SPVCNN(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.stage1 = nn.Sequential(
            ...
            FeatureExtractBlock(cs[0], cs[1], ks=3, stride=1, dilation=1),
            ResidualBlock(cs[1], cs[1], ks=3, stride=1, dilation=1),
        )

        self.stage2 = nn.Sequential(
            BasicConvolutionBlock(cs[1], cs[1], ks=2, stride=2, dilation=1),
            FeatureExtractBlock(cs[1], cs[2], ks=3, stride=1, dilation=1),
            ResidualBlock(cs[2], cs[2], ks=3, stride=1, dilation=1),
        )

        self.stage3 = nn.Sequential(
            BasicConvolutionBlock(cs[2], cs[2], ks=2, stride=2, dilation=1),
            FeatureExtractBlock(cs[2], cs[3], ks=3, stride=1, dilation=1),
            ResidualBlock(cs[3], cs[3], ks=3, stride=1, dilation=1),
        )

        self.stage4 = nn.Sequential(
            BasicConvolutionBlock(cs[3], cs[3], ks=2, stride=2, dilation=1),
            FeatureExtractBlock(cs[3], cs[4], ks=3, stride=1, dilation=1),
        ...
        self.up1 = nn.ModuleList([
            ...
            nn.Sequential(
                FeatureExtractBlock(cs[5] + cs[3], cs[5], ks=3, stride=1, dilation=1),
        ...
        self.up2 = nn.ModuleList([
            ...
            nn.Sequential(
                FeatureExtractBlock(cs[6] + cs[2], cs[6], ks=3, stride=1, dilation=1),
        ...
        self.up3 = nn.ModuleList([
            ...
            nn.Sequential(
                FeatureExtractBlock(cs[7] + cs[1], cs[7], ks=3, stride=1, dilation=1),
        ...
        self.up4 = nn.ModuleList([
            ...
            nn.Sequential(
                FeatureExtractBlock(cs[8] + cs[0], cs[8], ks=3, stride=1, dilation=1),
